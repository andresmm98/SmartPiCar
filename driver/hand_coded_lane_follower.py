...
def get_lane_lines(image, lines):
    ...
    for line in lines:
        for x1, y1, x2, y2 in line:
            if x1 == x2:
                logging.info('Ignoring vertical line \
...
def stabilize_steering_angle(curr_steering_angle, 
    ...
    logging.info(f'Calculated angle: {new_steering_angle}\n \
...
def test_video(video_file):
    ...
    video_overlay = cv2.VideoWriter(f"{video_file}_overlay.avi", 
    ...
    try:
        ...
        while cap.isOpened():
            ...
            cv2.imwrite(f'{video_file}_{i}_ \
                        {lane_follower.curr_steering_angle}.png', frame)
